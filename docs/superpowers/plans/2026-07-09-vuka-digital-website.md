...
- Installed `lucide-react` is `^1.23.0`, which removed all brand/logo icons (`Facebook`, `Instagram`, `Linkedin`, etc. no longer exist as exports — generic UI icons like `Menu`, `X`, `Mail`, `MapPin`, `Phone`, `Search`, `Globe2` etc. are unaffected). Task 5's `Footer.tsx` works around this with local inline SVG replicas of the old icon geometry rather than adding a new icon-package dependency. If any later task needs another brand/logo icon, use the same inline-SVG approach rather than assuming it's importable from `lucide-react`.
